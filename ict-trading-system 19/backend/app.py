...
async def evaluate_trade_risk(body: dict, db=Depends(get_db)):
    ...
    Every trade MUST pass through here. Decision is persisted to audit_log table.

    Body: { symbol, direction, strategy, proposed_entry, stop_loss_distance,
            proposed_risk_pct, source }
    """
    ...
    if account_state["stale"]:
        decision.warnings.append(
            f"Account state is STALE ({account_state['age_seconds']}s old) — risk inputs may be inaccurate"
        )

    result = decision.to_dict()

    # Phase 2: Persist to durable audit
    audit_svc = DurableAuditService(db)
    await audit_svc.log_risk_decision(result)
    await db.commit()
...
import time  # needed for freshness tracking


@app.get("/api/risk/audit-log")
async def risk_audit_log(limit: int = 50, category: str = None, db=Depends(get_db)):
    """Get durable audit log from database. Survives restarts.

    Optional ?category= filter: risk_decision, execution, kill_switch, breaker, signal_action
    """
    from backend.services.audit_service import DurableAuditService
    audit_svc = DurableAuditService(db)
...
# ══════════════════════════════════════════════════════════════════════
# EXECUTION GATE — Phase 3: No trade reaches broker without this
# ══════════════════════════════════════════════════════════════════════
...
async def execution_gate(body: dict, db=Depends(get_db)):
    ...
    Flow: request → risk evaluation → durable audit → broker submit/block

    Body: { symbol, direction, strategy, proposed_entry, stop_loss_distance,
            proposed_risk_pct, source }

    Result enums (Phase 5 — clear separation):
      policy_blocked — risk policy rejected
      policy_simulated — shadow mode, simulated only
      policy_ready_for_broker — approved, ready for manual broker submission
      broker_submitted — (future) order sent to broker
      broker_rejected — (future) broker rejected order
      broker_confirmed — (future) broker confirmed fill
    """
    ...
    if account_state["stale"] and account_state["equity"] == 0:
        return {
            ...
            "decision": {
                "approved": False,
                "blockers": ["Account state unavailable — cannot evaluate risk safely"],
                "warnings": [],
                "safety_state": {"degraded": True},
                "config_hash": "",
                "timestamp": datetime.now(timezone.utc).isoformat(),
            },
            "broker_status": None,
            "message": "Execution blocked: account state unavailable for risk evaluation",
        }
    ...
    # Phase 5: Map result to clear enums
    raw_result = gate_result.get("result", "unknown")
    if raw_result == "blocked":
        gate_result["result"] = "policy_blocked"
    elif raw_result == "simulated":
        gate_result["result"] = "policy_simulated"
    elif raw_result == "ready_for_broker":
        gate_result["result"] = "policy_ready_for_broker"

    # Phase 6: Add frontend-friendly fields
    gate_result["broker_status"] = None  # Not yet submitted to broker
    gate_result["message"] = _build_human_message(gate_result)
    gate_result["account_state_freshness"] = {
        "stale": account_state["stale"],
        "age_seconds": account_state["age_seconds"],
    }
    ...
    # Phase 2: Persist to durable audit
    audit_svc = DurableAuditService(db)
    await audit_svc.log_execution(gate_result.get("audit", {}))
    await db.commit()

    return gate_result


def _build_human_message(gate_result: dict) -> str:
    """Phase 6: Human-readable reason for frontend display."""
    ...
    elif result == "policy_simulated":
        return "Simulated (shadow mode) — not sent to broker"
    elif result == "policy_ready_for_broker":
        warnings = decision.get("warnings", [])
        if warnings:
            return f"Approved with warnings: {warnings[0]}"
        return "Approved — ready for broker submission"
    return "Unknown result"


@app.get("/api/execution/log")
async def execution_log(limit: int = 50, db=Depends(get_db)):
    """Get durable execution audit log."""
    from backend.services.audit_service import DurableAuditService
...
# ══════════════════════════════════════════════════════════════════════
# SCANNER → SIGNAL BRIDGE — Phase 5
# ══════════════════════════════════════════════════════════════════════

@app.post("/api/signals/promote-from-scan")
async def promote_scan_to_signal(body: dict, db=Depends(get_db)):
    """Promote a scanner result into a persistent Signal entity.

    Body: {
      symbol, direction, confidence, strategy, notes, metadata,
      source: "scanner"
    }

    Validates symbol against config, creates Signal, returns created signal.
    """
    ...
    valid, reason = get_risk_engine().validate_symbol(symbol, config.get("allowed_symbols", []))
    if not valid:
        return {"success": False, "error": reason}

    # Kill switch check
    if config.get("kill_switch", {}).get("active"):
        return {"success": False, "error": "Kill switch active — cannot create signals"}
    ...
    try:
        signal = await signal_svc.create_signal(
            symbol=symbol,
            strategy_name=body.get("strategy", "scanner_promotion"),
            side=direction,
...
# ══════════════════════════════════════════════════════════════════════
# ENHANCED SAFETY — Phase 6: Hardened circuit breaker endpoints
# ══════════════════════════════════════════════════════════════════════

@app.get("/api/safety/full-status")
async def full_safety_status(db=Depends(get_db)):
    """Phase 7: Complete safety state for frontend truth.

    Returns everything the frontend needs to be fully safety-aware.
    """
    ...
    return {
        ...
        "safety_summary": {
            "can_trade": (
                not config.get("kill_switch", {}).get("active", False)
                and not cb.tripped
            ),
            "blockers": _get_active_blockers(config, cb),
        },
    }


def _get_active_blockers(config: dict, cb) -> list[str]:
    """Get list of active trade blockers."""
    blockers = []
    if config.get("kill_switch", {}).get("active"):
        blockers.append("Kill switch active")
    if cb.tripped:
        blockers.append(f"Circuit breaker: {cb.consecutive_losses} consecutive losses")
...
async def hardened_breaker_reset(body: dict = {}, db=Depends(get_db)):
    """Phase 6: Reset circuit breaker with lifecycle validation.

    Refuses reset if kill switch active (unless force_override=true).

    Body: { reason, source, force_override }
    """
    from core.risk_policy import get_hardened_breaker
    from backend.dependencies import get_config_service
...
import json  # ensure json imported for promote endpoint
