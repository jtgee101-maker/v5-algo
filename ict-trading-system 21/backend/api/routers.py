...
async def kill_switch(
    ...
    require_admin(auth)
    cfg = get_config_service(db)
    # Persist kill switch in config (single source of truth)
    cfg.activate_kill_switch(req.reason)
    cfg._mode = "shadow"
    ...
    await GenericRepository(db, ConfigChangeRecord).create(
        config_type="runtime", config_key="kill_switch",
        old_value="false", new_value="true",
        changed_by=req.triggered_by, reason=req.reason,
    )
...
@control_router.post("/kill-switch/deactivate")
async def deactivate_kill_switch(
    ...
    require_admin(auth)
    cfg = get_config_service(db)
    cfg.deactivate_kill_switch()
    await GenericRepository(db, ConfigChangeRecord).create(
        config_type="runtime", config_key="kill_switch",
        old_value="true", new_value="false",
        changed_by="admin", reason="Kill switch deactivated",
    )
    return {"success": True, "kill_switch": cfg.get_config()["kill_switch"]}
...
async def set_mode(
    ...
    require_admin(auth)
    cfg = get_config_service(db)
    # Check kill switch before allowing mode change
    if cfg.is_kill_switch_active and req.mode != "shadow":
        raise HTTPException(400, "Kill switch is active. Deactivate it first.")
    try:
        old_mode = cfg.mode
        cfg.mode = req.mode
        await GenericRepository(db, ConfigChangeRecord).create(
