...
async def execution_gate(body: dict, db=Depends(get_db)):
    ...
    Result enums: policy_blocked | policy_simulated | policy_ready_for_broker
    """
    now_iso = datetime.now(timezone.utc).isoformat()
    ...
    try:
        ...
        if account_state["stale"] and account_state["equity"] == 0:
            return {
                "execution_id": exec_id, "approved": False,
                "result": "policy_blocked",
                "decision": {"approved": False,
                    "blockers": ["Account state unavailable"],
                    "warnings": [], "safety_state": {"degraded": True},
                    "config_hash": "", "timestamp": now_iso},
                "broker_status": None,
                "message": "Execution blocked: account state unavailable",
                "account_state_freshness": {"stale": True, "age_seconds": -1},
            }
        ...
        gate_result["broker_status"] = None
        ...
        try:
            from backend.services.audit_service import DurableAuditService
            await DurableAuditService(db).log_execution(gate_result.get("audit", {}))
            await db.commit()
        except Exception:
            pass
        ...
        return {
            ...
            "result": "policy_blocked",
            "decision": {"approved": False,
                "blockers": [f"Execution gate system error: {str(e)[:100]}"],
                "warnings": ["Execution gate degraded — blocking as safety precaution"],
                "safety_state": {"degraded": True}, "config_hash": "", "timestamp": now_iso},
            "broker_status": None,
            "message": f"Execution gate error: {str(e)[:80]}",
            "account_state_freshness": {"stale": True, "age_seconds": -1},
        }
...
async def execution_log(limit: int = 50, db=Depends(get_db)):
    """Get execution audit log. NEVER returns 404 — returns empty array if no data."""
    try:
        from backend.services.audit_service import DurableAuditService
        entries = await DurableAuditService(db).get_recent(category="execution", limit=limit)
        return {"entries": entries, "count": len(entries), "durable": True}
    except Exception:
        # Table might not exist yet or DB issue — return empty, not 500
        try:
