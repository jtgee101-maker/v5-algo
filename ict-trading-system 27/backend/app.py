...
# ══════════════════════════════════════════════════════════════════════
# MARKET DATA SNAPSHOT — real levels from OHLCV, no fake seeds
# ══════════════════════════════════════════════════════════════════════

@app.get("/api/market/snapshot/{symbol}")
async def market_snapshot(symbol: str, resolution: str = "1h", lookback: str = "5D"):
    """Get a real market data snapshot with calculated levels.

    Derives S/R, pivot, ATR from actual OHLCV candle data.
    NEVER returns fake/seeded values. If data unavailable, says so explicitly.

    Returns: { symbol, price, levels, source_status, is_live, timestamp }
    """
    sym = symbol.upper()
    now_iso = datetime.now(timezone.utc).isoformat()

    try:
        from core.market_data import get_market_data_service, SYMBOL_METADATA

        mds = get_market_data_service()

        # Get candle data (uses our fallback chain: TradeLocker → CoinGecko → Yahoo)
        candle_resp = await tl_candles(sym, resolution, lookback)
        candles = candle_resp.get("candles", [])
        source = candle_resp.get("source", "none")
        count = candle_resp.get("count", 0)

        # Classify data source
        if source == "tradelocker" and count > 10:
            source_status = "live"
            is_live = True
        elif source in ("coingecko", "yahoo") and count > 5:
            source_status = "fallback"
            is_live = False
        elif source == "synthetic" or count <= 1:
        ...
        # Calculate levels from real OHLCV
        levels = mds.calculate_levels(candles, sym)

        # Get current price
        current_price = 0
        if candles:
            current_price = float(candles[-1].get("close") or candles[-1].get("c", 0))

        # Price sanity check
        sanity = mds.validate_price_sanity(sym, current_price)
        ...
        return {
            "symbol": sym,
            "display_name": meta.get("display", sym),
            "type": meta.get("type", "unknown"),
            "instrument": meta.get("instrument", sym),
            "price": current_price,
            "price_sane": sanity.get("sane", False),
            "price_sanity_reason": sanity.get("reason", ""),
            "levels": levels if levels.get("levels_available") else None,
            "source": source,
            ...
            "data_quality": {
                ...
                "is_live": is_live,
                "candles": count,
                "levels_calculated": levels.get("levels_available", False),
                "price_sane": sanity.get("sane", False),
            },
        }
...
async def all_market_snapshots(resolution: str = "1h", lookback: str = "5D"):
    """Get market snapshots for all 6 symbols at once.

    This is the REAL data source for the Symbol Intelligence panel.
    """
    from core.market_data import ALL_SYMBOLS
    ...
    for sym in ALL_SYMBOLS:
        try:
            snap = await market_snapshot(sym, resolution, lookback)
            snapshots[sym] = snap
            status = snap.get("source_status", "unavailable")
            if status == "live":
                live_count += 1
            ...
            snapshots[sym] = {
                "symbol": sym, "price": 0, "levels": None,
                "source_status": "unavailable", "is_live": False,
            }
            unavailable_count += 1
    ...
    return {
        ...
        "summary": {
            ...
            "all_live": live_count == len(ALL_SYMBOLS),
...
@app.get("/api/market/symbol-map")
async def symbol_map():
    """Get the canonical symbol mapping — source of truth for all instruments."""
    from core.market_data import SYMBOL_METADATA
    return {
        "symbols": SYMBOL_METADATA,
        "count": len(SYMBOL_METADATA),
    }
