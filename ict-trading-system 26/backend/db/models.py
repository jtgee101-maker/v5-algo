...
class SignalMemoryRecord(Base):
    """Durable signal memory — survives restarts, tracks evolution.

    Every unique setup fingerprint gets one memory record.
    Versions are tracked. Duplicates reference this record.
    This is the source of truth for dedup decisions.
    """
    __tablename__ = "signal_memory"
    __table_args__ = (
        Index("idx_sigmem_fingerprint", "signal_fingerprint"),
        Index("idx_sigmem_symbol_active", "symbol", "is_active"),
        Index("idx_sigmem_last_seen", "last_seen_at"),
    )

    id = Column(String, primary_key=True, default=_uuid)  # memory_id
    signal_fingerprint = Column(String, nullable=False)
    symbol = Column(String, nullable=False)
    direction = Column(String, nullable=False)
    strategy = Column(String)
    source = Column(String)

    # Rounded levels used for fingerprint
    rounded_entry = Column(Float)
    rounded_stop = Column(Float)
    rounded_tp = Column(Float)

    # Quality snapshot
    classification = Column(String)  # actionable, candidate, low_confidence, rejected, invalid
    score_total = Column(Float, default=0)
    score_snapshot = Column(Text)    # JSON of score_components

    # Lifecycle
    first_seen_at = Column(String, nullable=False, default=_now_iso)
    last_seen_at = Column(String, nullable=False, default=_now_iso)
    last_emitted_scan_id = Column(String)
    emit_count = Column(Integer, default=1)
    suppress_count = Column(Integer, default=0)

    # Duplicate/version state
    duplicate_status = Column(String, default="unique")  # unique, near_duplicate, exact_duplicate, spam, conflicting
    current_signal_id = Column(String)      # latest signal entity linked to this memory
    parent_memory_id = Column(String)       # for version lineage
    version_number = Column(Integer, default=1)

    # Memory state
    memory_status = Column(String, default="active")  # active, superseded, invalidated, expired, archived
    is_active = Column(Integer, default=1)      # 1=active, 0=inactive (for fast queries)
    is_superseded = Column(Integer, default=0)
    invalidated_reason = Column(String)
