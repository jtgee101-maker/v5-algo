...
async def promote_scan_to_signal(body: dict, db=Depends(get_db)):
    """Promote a scanner result through the signal intelligence pipeline.

    NEW: Validates → scores → deduplicates → classifies before persisting.
    Garbage signals are rejected, not persisted as normal signals.

    Body: { symbol, direction, entry, stop_loss, take_profit, strategy, source,
            confidence, atr, current_price, volatility_regime, momentum,
            displacement_detected, fvg_present, fvg_fill_pct, metadata }
    """
    try:
        ...
        # Only persist actionable + candidate signals
        signal_id = None
        if result["action"] in ("create",) and result["classification"] in ("actionable", "candidate"):
            try:
                signal_svc = SignalService(db)
                signal = await signal_svc.create_signal(
                    symbol=body.get("symbol", "").upper(),
                    strategy_name=body.get("strategy", "scanner"),
                    side=body.get("direction", ""),
                    confidence=result["score_total"],
                    entry_price=float(body.get("entry", 0)),
                    stop_price=float(body.get("stop_loss", 0)),
                    take_profit_1=float(body.get("take_profit", 0)),
                ...
                if result.get("fingerprint"):
                    pipeline.deduplicator.register_signal(result["fingerprint"], signal_id)

            except Exception as e:
                result["persistence_error"] = str(e)[:100]

        return {
            "success": result["classification"] in ("actionable", "candidate"),
            "signal_id": signal_id,
            "classification": result["classification"],
            "score_total": result["score_total"],
            "score_components": result.get("score_components"),
...
async def dedup_stats():
    """Get signal deduplication statistics."""
    try:
        from core.signal_intelligence import get_signal_pipeline
        return get_signal_pipeline().deduplicator.get_stats()
    except Exception:
        return {"tracked_fingerprints": 0, "total_duplicates": 0}
